...
int main(int argc, char** argv)
{
	...
	unsigned int ppllBuildShader = CompileShader("kbufferBuild.vert", "kbufferBuild.frag");
	unsigned int ppllResolveShader = CompileShader("kbufferResolve.vert", "kbufferResolve.frag");
	...
	//2 buffers for K-buffer: one for storing data(color,depth,next); one for counter
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glDisable(GL_BLEND);
		glEnable(GL_DEPTH_TEST);
		timer.StopAndDisplay(window, "K-buffer");
